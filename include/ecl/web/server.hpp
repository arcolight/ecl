...
using http_cb_t      = http_cb;
using http_data_cb_t = http_data_cb;
...
public:
    ...
    {
        m_rq_cache.cache(buf, buf_size);
        int nparsed = http_parser_execute(&m_parser,
                                          &m_parser_settings,
                                          m_rq_cache.get_raw_rq(),
                                          m_rq_cache.get_raw_rq_size());
    }

    bool attach_resource(const char* url, i_resource&& res)
    {
        return true;
    }
